...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	private buildMonitorDownContent(
		...
		const downDurationMinutes = this.getDownDurationMinutes(monitor.downSince);
		const isEscalation = decision.notificationReason === "escalation" && downDurationMinutes !== null;
		const title = isEscalation ? `Escalation: ${monitor.name} is still down` : `Monitor Down: ${monitor.name}`;
		const summary = isEscalation
			? `Monitor "${monitor.name}" is still down after ${downDurationMinutes} minute${downDurationMinutes === 1 ? "" : "s"}.`
	...
	private getDownDurationMinutes(downSince?: string | null): number | null {
		if (!downSince) {
			return null;
		}

		const timestamp = Date.parse(downSince);
